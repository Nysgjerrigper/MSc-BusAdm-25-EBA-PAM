...
if not master_results:
    ...
    # Create the initial DataFrame with IDs
    results_df = pd.DataFrame(master_results)

    # --- START MODIFICATION ---

    # Define the mapping function (handles potential missing IDs)
    # You might already have this function defined for display, ensure it's available here
    def map_ids_to_names(id_list):
        # Add robustness: Check if it's actually a list or similar iterable
        if not isinstance(id_list, (list, tuple)):
            return id_list # Return as is if not a list (e.g., None, NaN)
        # Handle empty lists
        if not id_list:
            return []
        return sorted([player_name_map.get(p_id, f"ID:{p_id}") for p_id in id_list])

    # List of columns containing player ID lists that need conversion
    ...
    for col in id_list_columns:
        if col in results_df.columns:
            # Apply the mapping function to each list within the specified column
            # Make sure apply works correctly even if some cells are None or not lists
            results_df[col] = results_df[col].apply(lambda x: map_ids_to_names(x) if isinstance(x, (list, tuple)) else x)
        else:
            print(f"Warning: Column '{col}' not found in results_df for name conversion.")

    print("ID to Name conversion complete.")
    # --- END MODIFICATION ---


    # Display summary per GW (This existing code will now show the names correctly)
    for index, row in results_df.iterrows():
        gw = row['gameweek']
        print(f"\n--- GW {gw} Summary ---")
        print(f"  Chip Played: {row['chip_played']}")
        # Directly print the columns as they now contain sorted name lists
        print(f"  Transfers In ({len(row['transfers_in'])}): {row['transfers_in']}")
        print(f"  Transfers Out ({len(row['transfers_out'])}): {row['transfers_out']}")
        print(f"  FT Available (Start): {row['q_start']}")
        print(f"  Penalized Transfers (Hits): {row['alpha']}")
        # Access the first element for captain/vice, checking if the list is not empty
        print(f"  Captain: {row['captain'][0] if row['captain'] else 'None'}")
        print(f"  Vice-Captain: {row['vice_captain'][0] if row['vice_captain'] else 'None'}")
        print(f"  Budget End: {row['budget_end']:.1f}")
        obj_val_str = f"{row['objective_value']:.2f}" if row['objective_value'] is not None else "N/A"
        print(f"  Objective Value (Sub-problem): {obj_val_str}")
        # Optional: Display full squad/lineup (already contains names)
        # print(f"  Squad ({len(row['squad'])}): {row['squad']}")
        # print(f"  Lineup ({len(row['lineup'])}): {row['lineup']}")


    # Save the MODIFIED results DataFrame (now containing names)
    try:
        # Consider saving to a different file name to distinguish
