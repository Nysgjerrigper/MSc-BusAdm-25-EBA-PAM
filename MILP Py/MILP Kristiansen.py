...
# Increase CBC logging and threads if needed (adjust based on your system)
# solver = pulp.PULP_CBC_CMD(msg=True, threads=4, timeLimit=180) # Log, 4 threads, 3 min limit

try:
    status = model.solve(solver) # Pass specific solver if needed
...
if status == pulp.LpStatusOptimal or status == pulp.LpStatusNotSolved : # Also show results if stopped by timelimit etc.
    ...
    def get_selected_players(var_dict, gw, player_map):
        selected_ids = [p_ for p_ in var_dict if var_dict[p_][gw].varValue > 0.99]
        names = [player_map.get(p_id, f"Unknown ID: {p_id}") for p_id in selected_ids]
        return pd.DataFrame({'player_id': selected_ids, 'name': names}) if selected_ids else pd.DataFrame(columns=['player_id', 'name'])

    # --- Display Key Decisions for Each Gameweek ---
    for t_ in t:
        print(f"\n--- Gameweek {t_} Decisions ---")

        squad_df = get_selected_players(x, t_, player_name_map)
        print(f"\nSquad (GW{t_}): {len(squad_df)} players")
        if not squad_df.empty: print(squad_df.to_string(index=False))

        lineup_df = get_selected_players(y, t_, player_name_map)
        print(f"\nStarting Lineup (GW{t_}): {len(lineup_df)} players")
        if not lineup_df.empty: print(lineup_df.to_string(index=False))

        captain_df = get_selected_players(f, t_, player_name_map)
        if not captain_df.empty: print(f"Captain: {captain_df['name'].iloc[0]} (ID: {captain_df['player_id'].iloc[0]})")

        vc_df = get_selected_players(h, t_, player_name_map)
        if not vc_df.empty: print(f"Vice-Captain: {vc_df['name'].iloc[0]} (ID: {vc_df['player_id'].iloc[0]})")

        # Gamechip status
        if w[t_].varValue > 0.99: print("WILDCARD ACTIVE")
        if b[t_].varValue > 0.99: print("BENCH BOOST ACTIVE")
        if r[t_].varValue > 0.99: print("FREE HIT ACTIVE")
        tc_df = get_selected_players(is_tc, t_, player_name_map)
        if not tc_df.empty: print(f"TRIPLE CAPTAIN ACTIVE on: {tc_df['name'].iloc[0]} (ID: {tc_df['player_id'].iloc[0]})")


        # Transfers made *in* this gameweek (relevant for t >= 2)
        if t_ > min(t):
            transfers_in_df = get_selected_players(e, t_, player_name_map)
            if not transfers_in_df.empty:
                print(f"\nTransfers IN ({len(transfers_in_df)}):")
                print(transfers_in_df.to_string(index=False))

            transfers_out_df = get_selected_players(u, t_, player_name_map)
            if not transfers_out_df.empty:
                print(f"\nTransfers OUT ({len(transfers_out_df)}):")
        ...
        print(f"Remaining Budget (End of GW{t_}): {v[t_].varValue:.1f}") # Scaled value

elif status == pulp.LpStatusInfeasible:
    print("ERROR: Model is infeasible. Check constraints and data.")
    # You could try relaxing constraints or using IIS (Infeasible Subsystem) analysis if supported by the solver
