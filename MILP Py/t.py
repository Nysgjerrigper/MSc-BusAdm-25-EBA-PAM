...
if USE_INITIAL_TEAM and USE_NAMES_NOT_IDS:
    ...
    for player_name in INITIAL_TEAM_NAMES:
        if player_name in name_to_id_map:
            INITIAL_TEAM_IDS.append(name_to_id_map[player_name])
        else:
            print(f"WARNING: Player '{player_name}' not found in data!")
            
    print(f"Converted {len(INITIAL_TEAM_IDS)} player names to IDs for initial team")
...
for current_gw in range(START_GAMEWEEK, MAX_GAMEWEEK + 1):
    ...
    if current_gw == START_GAMEWEEK:
        if USE_INITIAL_TEAM:
            # Validate the initial team
            valid_ids = [pid for pid in INITIAL_TEAM_IDS if pid in p]
            if len(valid_ids) != 15:
                print(f"WARNING: Initial team should have 15 players but found {len(valid_ids)} valid IDs")
                
            # Initialize with provided team
            previous_squad_dict = {pid: 1 for pid in valid_ids}
            previous_budget = INITIAL_BUDGET
            previous_ft = INITIAL_FT
            
            # Force the model to use this initial squad
            for p_ in p:
                if p_ in valid_ids:
                    model += x[p_][t1_sub] == 1, f"InitialTeam_Force_{p_}"
                else:
                    model += x[p_][t1_sub] == 0, f"InitialTeam_Exclude_{p_}"
                    
            # Allow transfers in first gameweek (unlike default behavior)
            
            print(f"Using initial team with {len(valid_ids)} players and budget {previous_budget:.1f}")
        else:
            # Original behavior for starting from scratch
...
if not master_results:
    ...
    # Save results
    try:
        output_csv_name = f"Squad Selection t, W{START_GAMEWEEK}-{MAX_GAMEWEEK},SHL{SUB_HORIZON_LENGTH}.csv"
