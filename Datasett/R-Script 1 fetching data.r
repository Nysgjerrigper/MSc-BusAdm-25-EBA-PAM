...
# Hent data for gameweeks 1 til 31 (18.04.2025)
sistegw <- 31
for (i in 1:sistegw) {
...
# Define on-field positions
onfield_positions <- c("GK", "DEF", "MID", "FWD")

# Identify assistant managers as those with positions not in onfield_positions
assistant_managers <- Sesong_24_25 %>%
  filter(!(position %in% onfield_positions)) %>%
  pull(name) %>%
  unique()

# Display identified assistant managers
cat("Identified", length(assistant_managers), "assistant managers\n")

# Filter out assistant managers from the dataset
Sesong_24_25 <- Sesong_24_25 %>%
  filter(!(name %in% assistant_managers))

# Sjekk om noen assistentmanagere fortsatt finnes i datasettet
Sesong_24_25 |> filter(name %in% assistant_managers)
...
## Sesongvis ====
write_csv(Sesong_22_23, file = "Datasett/Sesong 22 til 23.csv")
write_csv(Sesong_23_24, file = "Datasett/Sesong 23 til 24.csv")
write_csv(Sesong_24_25, file = "Datasett/Sesong 24 til 25.csv")
cat("Lagret filene\n")

## Samlet ====

# Samlet sett
write_csv(sesongogspiller_id, file = "Datasett/Sesong id, alle tre sesonger(22-24).csv")
cat("Lagret filen\n")

# Stigende rekkefølger GW
write_csv(stigenderekkefølge, "Datasett/Stigende GW, alle tre sesonger(22-24).csv")

# Alternativt Datasett 
write_csv(alternativtsammensatt, "Datasett/Ekstra kolonner, stigende GW, alle tre sesonger(22-24), heltall.csv")
